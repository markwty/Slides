...
const long SlidesFrame::ID_RADIOBOX1 = wxNewId();
...
const long SlidesFrame::ID_INCLUDEMACROCHKBOX = wxNewId();
...
SlidesFrame::SlidesFrame(wxWindow* parent,wxWindowID id)
{
    ...
    VideoRadioBox = new wxRadioBox(this, ID_VIDEORADIOBOX, _("Video"), wxPoint(104,328), wxSize(144,114), 4, __wxRadioBoxChoices_1, 1, 0, wxDefaultValidator, _T("ID_VIDEORADIOBOX"));
    ...
    OptionsRadioBox = new wxRadioBox(this, ID_RADIOBOX1, _("Browse Options"), wxPoint(568,264), wxSize(112,96), 3, __wxRadioBoxChoices_2, 1, 0, wxDefaultValidator, _T("ID_RADIOBOX1"));
    ...
    IncludeMacroChkBox = new wxCheckBox(this, ID_INCLUDEMACROCHKBOX, _("Include macro"), wxPoint(0,400), wxDefaultSize, 0, wxDefaultValidator, _T("ID_INCLUDEMACROCHKBOX"));
    IncludeMacroChkBox->SetValue(false);
    IncludeMacroChkBox->SetToolTip(_("Tick to include code for slideshow\n(issue, only applicable to \'Background\' & \'Mix\')"));
    ...
    rk.SetValue("SlidesProject.exe", 11001);
    ...
    Connect(ID_WEBVIEW, wxEVT_WEBVIEW_LOADED, wxWebViewEventHandler(SlidesFrame::ExecuteScript));
...
{
    vector<wxString> platforms = {Genius, AZLyrics, WorshipTogether, KLove, LyricsOnDemand, Lyrics};
...
void SlidesFrame::ShowSlides(wxString foldername)
{
    ...
    while (cont)
    {
        ...
        FileListBox->Append(filenamestr);
    ...
    {
        index = FileListBox->FindString(*it);
...
void SlidesFrame::Save()
{
    ...
    {
        line = *it2 << "\n";
        wxofsFile2.Write(line, line.Len());
    }
    wxofsFile2.Close();
}
...
void SlidesFrame::ScriptProgression()
{
    ...
    if (through)
    {
        ...
        {
            webView-> LoadURL("http://www.klove.com/music/lyrics/?s=Songs&t=All#music-search-form");
        }
        ...
        {
            webView-> LoadURL("https://www.worshiptogether.com/");
        }
        else
        {
...
void SlidesFrame::ExecuteScript(wxWebViewEvent& event)
{
    ...
    wxMessageBox(url);
    if (url.compare("https://profile.ccli.com/account/signin?appContext=OLR&returnUrl=https%3a%2f%2folr.ccli.com%3a443%2f") == 0)
    {
        webView-> RunScript(
        "\
        document.getElementById('EmailAddress').value = 'faithsanctuaryoffice@gmail.com';\
        document.getElementById('Password').value = 'faith67449954';\
        document.getElementById('sign-into-account').submit();\
        ");
    }
    else if (url.compare("https://olr.ccli.com/") == 0)
    {
        ...
        if (webstate == 1)
        {
            webView-> RunScript(
            "\
            document.getElementById('SearchTerm').value = \"" + SearchTextCtrl2->GetLineText(0) + "\";\
            document.getElementById('frmSearchTerm').submit();\
            ");
        }
        else if (webstate == 2)
        {
            wxString id = SongsListCtrl->GetItemText(curr_index, 2);
            while (id.compare("") == 0){
                if (curr_index < SongsListCtrl->GetItemCount() - 1)
                {
                    curr_index++;
                    id = SongsListCtrl->GetItemText(curr_index, 2);
                }
                else
                {
                    CreateDatabaseBtn->Enable(true);
                    webstate = -1;
                    SetStatusText("Songs reported (with given licensing)", 0);
                    return;
                }
            }
            webView-> RunScript(
            "\
            document.getElementById('SearchTerm').value = \"" + id + "\";\
            document.getElementById('frmSearchTerm').submit();\
            ");
        }
    }
    else if (url.find("https://olr.ccli.com/search/results?SearchTerm=") == 0)
    {
        if (webstate == 1)
        {
            ...
            webView-> RunScript(
            "\
                var cnt = document.getElementsByClassName('eight mobile-two columns').length;\
                cnt;\
            ", output);
            ...
            "\
                ...
                var output = [];\
                var elements = document.getElementsByClassName('eight mobile-two columns');\
                ...
                var cnt = 0;\
                elements = document.getElementsByClassName('secondarySongAttributes');\
                for (var i = 0; i < elements.length; i++)\
                {\
                    if (trim(elements[i].innerText).substring(1,10) == 'Copyright')\
                    {\
                        output.push(elements[i].innerText);\
                    }\
                    else if (trim(elements[i].innerText).substring(0,14) == 'Lyrics Preview')\
                    {\
                        output.push(elements[i].innerText);\
                        cnt++;\
                        if (cnt == cap)\
            ...
            for (int i = 0; i < cnt; i++)
            {
                ...
                line = tkz2.GetNextToken().Trim(false).Trim(true);
                while (line.IsEmpty() && tkz2.HasMoreTokens())
                {
                    line = tkz2.GetNextToken().Trim(false).Trim(true);
                }
                content = line;
                ...
                content += "\n" + line.substr(13);
                songsDetails[i].push_back(content);
            }
            for (int i = 0; i < cnt; i++)
            {
                wxStringTokenizer tkz2(tkz.GetNextToken(), wxT("\n"));
                tkz2.GetNextToken();
                content = tkz2.GetString();
                songsDetails[i].push_back(content);
                wxStringTokenizer tkz3(tkz.GetNextToken(), wxT("\n"));
                wxString token;
                tkz3.GetNextToken();
                content = tkz3.GetNextToken();
                int max_lines = 4;//arbitrary
                if (content.Trim(false).Trim(true).compare("No lyrics are currently available") == 0)
                {
                    content = "";
                }
                else
        ...
        {
            SetStatusText("Reporting " + SongsListCtrl->GetItemText(curr_index, 0), 0);
            webView-> RunScript(
            "\
            ...
            var elements = document.getElementsByName('DigitalCount');\
            for (var i = 0; i < elements.length; i++)\
            {\
                elements[i].value = 1;\
            }\
            var elements = getElementsByClassName(document.body, 'button small');\
            for (var i = 0; i < elements.length; i++)\
            {\
                if (elements[i].innerText == 'Add to Report')\
                {\
                    elements[i].click();\
                    break;\
                }\
            ...
            {
                curr_index++;
                webView-> LoadURL("https://olr.ccli.com/");
            }
            else
            {
                CreateDatabaseBtn->Enable(true);
    ...
    else if (url.compare("http://www.klove.com/music/lyrics/?s=Songs&t=All#music-search-form") == 0 \
             || url.compare("https://www.lyrics.com/") == 0 \
             || url.compare("https://www.worshiptogether.com/") == 0 \
             || url.compare("https://www.google.com/") == 0)
    {
        SetStatusText("Extracting lyrics from "+ selected_platform, 0);
        ...
        //Requires exact song name
        {
            webView-> RunScript(
            "\
            document.getElementById('txtSearchFilter').value = \"" + tkz.GetNextToken() + "\";\
            document.getElementById('ctl00_ctl00_cphMain_cphMainTop_ucMusicSearch_lbSearch').click();\
            ");
        }
        else if (selected_platform.compare(Lyrics) == 0)
        {
        ...
        else if (selected_platform.compare(WorshipTogether) == 0)
        {
            webView-> RunScript(
            "\
            document.getElementById('_ctl0_search').value = \"" + tkz.GetNextToken() + " " + songsDetails[curr_index][2] + "\";\
    ...
    else if (url.find("http://www.klove.com/music/lyrics/?q") == 0 \
             || url.find("https://worship-songs-resources.worshiptogether.com/") == 0 \
             ...
             || url.find("https://www.google.com/search") == 0)
    {
        ...
        if (selected_platform.compare(KLove) == 0)
        {
            ...
            "\
            var through = 0;\
            var elements = document.getElementById('music-search-results').getElementsByTagName('a');\
        ...
        else if (selected_platform.compare(Lyrics) == 0)
        {
            ...
            var through = 0;\
            var elements = getElementsByClassName(document.body, 'lyric-body');\
            if (elements.length != 0)\
            {\
                elements[0].click();\
                through = 1;\
        ...
        else if (selected_platform.compare(WorshipTogether) == 0)
        {
            ...
            var through = 0;\
            var elements = getElementsByClassName(document.body, 'item song sli_wrapper sli_col1');\
            if (elements.length != 0)\
            {\
                elements[0].getElementsByTagName('a')[1].click();\
                through = 1;\
            }\
            through;\
            ", output);
        ...
        else
        {
            ...
            var through = 0;\
            var elements = getElementsByClassName(document.body, 'LC20lb');\
            if (elements.length != 0)\
            {\
                elements[0].click();\
                through = 1;\
        ...
        delete output;
             ...
             || url.find("http://www.klove.com/music/artists") == 0 \
             ...
             || url.find("https://www.worshiptogether.com/songs") == 0)
    {
        ...
        if (selected_platform.compare(AZLyrics) == 0 && url.find("https://www.azlyrics.com/lyrics/") == 0)
        {
            ...
            var output = '';\
            var inner = getElementsByClassName(document.body, 'ringtone')[0].innerText;\
            var elements = document.getElementsByTagName('div');\
            for (var i = 0; i < elements.length; i++)\
            {\
                if (elements[i].innerText == inner)\
                {\
                    output = elements[i + 5].innerText;\
                    break;\
                };\
            }\
            output;\
        ...
        else if (selected_platform.compare(Genius) == 0 && url.find("https://genius.com/") == 0)
        {
            ...
            var output = '';\
            var elements = getElementsByClassName(document.body, 'lyrics');\
            if (elements.length != 0)\
            {\
        ...
        else if (selected_platform.compare(KLove) == 0 && url.find("http://www.klove.com/music/artists") == 0)
        {
            webView-> RunScript(
            "\
            document.getElementById('ctl00_cphMain_lblLyrics').innerText;\
            ", lyrics);
        }
        ...
        else if (selected_platform.compare(LyricsOnDemand) == 0 && url.find("http://www.lyricsondemand.com/") == 0)
        {
            webView-> RunScript(
            "\
            function getElementsByClassName(node, classname) {\
            ...
            var output = '';\
            var elements = getElementsByClassName(document.body, 'lcontent');\
            if (elements.length != 0)\
            {\
                output = elements[0].innerText;\
            }\
        ...
        else if (selected_platform.compare(Musixmatch) == 0 && url.find("https://www.musixmatch.com/lyrics/") == 0)
        {
            ...
            var output = '';\
            var elements = getElementsByClassName(document.body, 'mxm-lyrics__content');\
            for (var i = 0; i < elements.length; i++)\
            {\
        ...
        {
            //Cannot access/load full HTML of page
            webView-> RunScript(
            "\
            function getElementsByClassName(node, classname) {\
                var a = [];\
            ...
            var output = '';\
            var elements = getElementsByClassName(document.body, '_1zlI0');\
            for (var i = 0; i < elements.length; i++)\
            {\
        ...
        else if (selected_platform.compare(WorshipTogether) == 0 && url.find("https://www.worshiptogether.com/songs") == 0)
        {
            ...
            var output = '';\
            document.getElementById('chordPro_nav').getElementsByTagName('a')[1].click();\
            var elements = document.getElementsByTagName('div');\
            for (var i = 0; i < elements.length; i++)\
            {\
                if (elements[i].className == 'chord-pro-line')\
                {\
                    var lyric_elements = getElementsByClassName(elements[i], 'chord-pro-lyric');\
                    output += '\\n';\
                    for (var ii = 0; ii < lyric_elements.length; ii++)\
                    {\
                        if (lyric_elements[ii].innerText.indexOf('Copyright') !== -1)\
                        {\
                            break;\
                        }\
                        else\
                        {\
                            output += lyric_elements[ii].innerText;\
                        }\
                    }\
                }\
                else if (elements[i].className == 'chord-pro-br')\
                {\
                    output += '\\n ';\
                }\
            }\
...
void SlidesFrame::OnCreateSlidesBtnClick(wxCommandEvent& event)
{
    ...
    try
    {
            ...
            for (int ii = cnt + 1; ii <= cnt2; ii++)
            {
                ...
                for (int iii = 1; iii <= count2; iii++)
                {
                    ...
                    if (textframe.GetProperty("HasText").GetBool())
                    {
                        ...
                        {
                            textframe.PutProperty("TextRange.Text", info[3 + ii - cnt]);
        ...
        if (IncludeMacroChkBox->GetValue() && \
...
void SlidesFrame::OnImportCreateBtnClick(wxCommandEvent& event)
{
    ...
    try
    {
        ...
        else
        {
            ...
            if(configmap["Default videos directory"].find("\\") == -1)
            {
...
void SlidesFrame::FilterSearch(wxCommandEvent& event)
{
    ...
    SlidesFrame::ShowSlides(configmap["Default songs directory"]);
	int num_items = FileListBox->GetCount();
	for (int i = 0; i < num_items; i++)
	{
        FileListBox->Delete(0);
